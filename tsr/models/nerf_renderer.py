...
class TriplaneNeRFRenderer(BaseModule):
    ...
    def query_triplane(
        ...
        if triplane.ndim == 5 and triplane.shape[0] == 1:
            triplane = triplane.squeeze(0)  # shape: [3, C, H, W]
    ...
    def _forward(
        ...
        **kwargs,
        ...
        xyz = rays_o[:, None, :] + z_vals[..., None] * rays_d[..., None, :]
        ...
        accum_prod = torch.cat(
            [torch.ones_like(alpha[:, :1]), torch.cumprod(1 - alpha[:, :-1] + eps, dim=-1)],
            dim=-1,
        )
